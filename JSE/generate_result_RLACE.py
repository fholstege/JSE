...
def main(dataset, dataset_setting,spurious_ratio, demean, pca, k_components,rank,  solver, lr, batch_size, per_step, early_stopping,  epochs, weight_decay, balanced_training_main, balanced_training_concept, device_type, save_results, seed, folder, num_iters = 50000, optimizer_class = torch.optim.SGD,   epsilon = 0.01, use_standard_ERM_settings_after = True, use_standard_INLP_settings_after=True, single_finetuned_model=False):
    ...
    X_train_transformed = torch.matmul(X_train, P_svd)
    X_val_transformed = torch.matmul(X_val, P_svd)
    X_test_transformed = torch.matmul(X_test, P_svd)
    ...
    # reset_X with the training/validation data
    data_obj.reset_X(X_train_transformed, X_val_transformed, batch_size=batch_size, include_weights=balanced_training_main,train_weights = main_weights_train, val_weights = main_weights_val, only_main=True)


    
     # Train the model to get main
    set_seed(seed)
    main_model = return_linear_model(X_train_transformed.shape[1], 
    ...
    if save_results:
        ...
        filename = 'results/{}/{}_{}_seed_{}'.format(folder, dataset, 'RLACE', seed)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--single_finetuned_model', type=str, help='single finetuned model to use')
    ...
    main(dataset, dataset_setting,spurious_ratio, demean, pca, k_components,rank,  solver, lr, batch_size, per_step, early_stopping, epochs, weight_decay,balanced_training_main, balanced_training_concept, device_type, save_results, seed, folder, use_standard_INLP_settings_after=use_standard_INLP_settings_after, single_finetuned_model=single_finetuned_model)