...
def main(method, sims, dataset, dataset_setting, spurious_ratio_list, demean, pca, k_components, alpha,  batch_size, solver, lr,weight_decay,  early_stopping, epochs, per_step, device_type, save_results, run_seed, single_finetuned_model,group_weighted=False, null_is_concept=False, rafvogel_with_joint=False, orthogonality_constraint=False, baseline_adjust=True, delta=0.0, eval_balanced=True, rank_RLACE=1, use_standard_settings=True, concept_first=True, remove_concept=True, weight_misclassified=2, n_for_estimation='all', use_smaller_set=False,group_subsample=False, C=0, eta_g=0.1):
    ...
    if method == 'ERM':
        ...
        if use_standard_settings and use_smaller_set: # overrides the above
            settings = 'standard_ERM_settings_smaller_dataset'
        if use_standard_settings and group_subsample:
            settings =  'standard_ERM_settings_SUBG'

       
        
      
        
        lr = optimizer_info[settings][dataset]['lr']
        weight_decay = optimizer_info[settings][dataset]['weight_decay']
        batch_size = optimizer_info[settings][dataset]['batch_size']
        ...
        terminal_call = 'python3 generate_result_ERM.py ' + dataset_str + ' ' +   dataset_setting_str + ' ' + demean_str + ' ' + pca_str + ' ' + k_components_str + ' ' + alpha_str + ' ' + batch_size_str + ' ' + solver_str + ' ' + lr_str + ' ' + weight_decay_str + ' ' + early_stopping_str + ' ' + epochs_str + ' ' + balanced_training_main_str + ' ' + ' ' + group_weighted_str + ' ' + per_step_str + ' ' + device_type_str + ' ' + save_results_str  + ' ' + n_for_estimation_str + ' ' + use_smaller_set_str + ' ' + group_subsample_str

    elif method == 'GDRO':


        if use_standard_settings:
            settings_GDRO='standard_GDRO_settings'
        if use_standard_settings and use_smaller_set:
            settings_GDRO='standard_GDRO_settings_smaller_dataset'

        lr = optimizer_info[settings_GDRO][dataset]['lr']
        weight_decay = optimizer_info[settings_GDRO][dataset]['weight_decay']
        batch_size = optimizer_info[settings_GDRO][dataset]['batch_size']
        C = optimizer_info[settings_GDRO][dataset]['C']
        ...
        use_smaller_set_str = '--use_smaller_set={}'.format(use_smaller_set)
        n_for_estimation_str = '--n_for_estimation={}'.format(n_for_estimation)
        print('use smaller set: ', use_smaller_set_str)
        print('balanced_training_main_str: ', balanced_training_main_str)


        terminal_call = 'python3 generate_result_GDRO.py ' + dataset_str + ' ' +   dataset_setting_str + ' ' + demean_str + ' ' + pca_str + ' ' + k_components_str + ' ' + alpha_str + ' ' + batch_size_str + ' ' + solver_str + ' ' + lr_str + ' ' + weight_decay_str + ' ' + early_stopping_str + ' ' + epochs_str + ' ' + balanced_training_main_str + ' ' + per_step_str + ' ' + device_type_str + ' ' + save_results_str  + ' ' + C_str + ' ' + eta_g_str + ' ' + use_smaller_set_str + ' '+  n_for_estimation_str
        ...
        if use_standard_settings:
            ...
            weight_misclassified = optimizer_info['standard_JTT_settings'][dataset]['weight_misclassified']
            print('Using standard JTT settings')
        ...
        terminal_call = 'python generate_result_JTT.py ' + dataset_str + ' ' +  dataset_setting_str + ' ' + demean_str + ' ' + pca_str + ' ' + k_components_str + ' ' + alpha_str + ' ' + batch_size_str + ' ' + solver_str + ' ' + lr_str + ' ' + weight_decay_str + ' ' + early_stopping_str + ' ' + epochs_str + ' ' + balanced_training_main_str + ' ' + weight_misclassified_str + ' ' + per_step_str + ' ' + device_type_str + ' ' + save_results_str
    ...
    # create the terminal call for JSE
        ...
        if use_standard_settings:
            settings = 'standard_JSE_settings'
            if use_smaller_set:
                settings = 'standard_JSE_settings_smaller_dataset'
    ...
    # create the terminal call for RLACE
        ...
        terminal_call = 'python3.10 generate_result_LEACE.py ' + dataset_str + ' ' + dataset_setting_str  + ' ' + demean_str + ' ' + pca_str + ' ' + k_components_str + ' '  + batch_size_str + ' ' + solver_str + ' ' + lr_str + ' ' + weight_decay_str + ' ' + early_stopping_str + ' ' + epochs_str + ' ' + balanced_training_main_str + ' ' + balanced_training_concept_str + ' ' + per_step_str + ' ' + device_type_str + ' ' + save_results_str
    ...
    for spurious_ratio in spurious_ratio_list:
        ...
        for sim in range(sims):
            ...
            # set the seed_str 
            seed_str = '--seed={}'.format(sim )

            # single finetuned model or not?
            single_finetuned_model_str =  '--single_finetuned_model={}'.format(single_finetuned_model)
            ...
            terminal_call_sim = terminal_call_sim + ' ' + spurious_ratio_str + ' ' + single_finetuned_model_str
            ...
            if save_results:
                ...
                filename = '{}_{}_seed_{}'.format( dataset, method, sim )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--weight_misclassified", type=float, default=2.0, help="weight for misclassified samples")
    parser.add_argument("--n_for_estimation", type=str, default="all", help="number of samples to use for estimation")
    parser.add_argument("--use_smaller_set", type=str, default="False", help="whether to use smaller set")
    parser.add_argument("--group_subsample", type=str, default='False', help='to implement SUBG method')
    ...
    parser.add_argument('--C', type=float, default=0.0, help='C for GDRO')
    ...
    main(method, sims, dataset, dataset_setting, spurious_ratio, demean, pca, k_components, alpha,  batch_size, solver, lr,weight_decay,  early_stopping, epochs, per_step, device_type, save_results, run_seed,single_finetuned_model, null_is_concept=null_is_concept, group_weighted=group_weighted, rafvogel_with_joint=rafvogel_with_joint, orthogonality_constraint=orthogonality_constraint, baseline_adjust=baseline_adjust, delta=delta, eval_balanced=eval_balanced,  rank_RLACE=rank_RLACE, use_standard_settings=use_standard_settings, concept_first=concept_first, remove_concept=remove_concept, weight_misclassified=weight_misclassified, n_for_estimation=n_for_estimation, use_smaller_set=use_smaller_set, group_subsample=group_subsample, C=C, eta_g=eta_g)
