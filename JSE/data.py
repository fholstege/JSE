...
class Dataset_obj():
    ...
    def get_groups( y_c, y_m):
        ...
        return groups
    @staticmethod
    def get_groups_second_concept(y_c, y_m, y_z):
        # get the groups
        groups = []
        for i in range(len(y_c)):
            group = return_group_second_concept(y_c[i], y_m[i], y_z[i])
            groups.append(group)
    ...
    def get_group_sizes_split(self, subset_size_1, subset_size_0, spurious_ratio , set_spurious_ratio_same_for_both = True):
        """
        ...
        # if spurious ratio is not the same for both, set the spurious ratio for the other class
        if not set_spurious_ratio_same_for_both:
            spurious_ratio = 1 - spurious_ratio
    ...
    def set_sample_split(self, y_m_split, y_c_split, n_split, spurious_ratio, p_m,  seed, set_spurious_ratio_same_for_both = True, equal_to_smallest=False):
        """
        ...
        # get the group sizes
        if equal_to_smallest:
            n_sample_0_0 = torch.where((y_m_split == 0)& (y_c_split == 0), 1, 0).sum()
            n_sample_0_1 = torch.where((y_m_split == 0)& (y_c_split == 1), 1, 0).sum()
            n_sample_1_0 = torch.where((y_m_split == 1)& (y_c_split == 0), 1, 0).sum()
            n_sample_1_1 = torch.where((y_m_split == 1)& (y_c_split == 1), 1, 0).sum()
            n_min = torch.min(torch.tensor([n_sample_0_0,n_sample_0_1, n_sample_1_0, n_sample_1_1] ))
            n_1_1 = n_1_0 = n_0_1 = n_0_0 = n_min
            print('smallest group size: ', n_1_1)

        else:
            n_1_1, n_1_0, n_0_0, n_0_1 = self.get_group_sizes_split(y_m_1, y_m_0, spurious_ratio, set_spurious_ratio_same_for_both = set_spurious_ratio_same_for_both)
    ...
    def reset_X(self, new_X_train, new_X_val, batch_size, workers=0, shuffle=True, only_main = False, reset_X_objects = True, include_weights = False, train_weights = None, val_weights = None, concept_first=True, add_indeces=False):
        """
        ...
        if not only_main:
            self.concept_main_loader = self.create_loaders(batch_size, workers, shuffle=shuffle, with_concept=True, include_weights= include_weights, train_weights = train_weights, val_weights = val_weights, concept_first=concept_first, add_indeces=False)
            self.concept_loader = self.create_loaders(batch_size, workers, shuffle=shuffle, with_concept=False, which_dependent = 'concept', include_weights= include_weights, train_weights = train_weights, val_weights = val_weights, add_indeces=add_indeces)
        
        # reset the loaders involving main labels
        self.main_loader = self.create_loaders(batch_size, workers, shuffle=shuffle, with_concept=False, which_dependent='main', include_weights= include_weights, train_weights = train_weights, val_weights = val_weights, add_indeces=add_indeces)
    ...
    def get_group_weights(self, normalized=True, second_concept=False):
        """
        ...
        # get the groups
        if second_concept:
            groups_train = self.get_groups_second_concept(self.y_c_train, self.y_m_train, self.y_c_2_train)
            groups_val = self.get_groups_second_concept(self.y_c_val, self.y_m_val, self.y_c_2_val)
        else:
        ...
        if second_concept:
            n_per_group_train = self.get_n_per_class( torch.Tensor(groups_train).int(), classes= [0, 1,2,3, 4, 5, 6, 7])
            n_per_group_val = self.get_n_per_class( torch.Tensor(groups_val).int(), classes= [0, 1,2,3, 4, 5, 6, 7])
        else:
    ...
    def create_loaders(self, batch_size, workers, shuffle=True, with_concept=True, which_dependent='main', include_weights=False, train_weights = None, val_weights = None, concept_first=True, add_indeces=False):
        """ 
        ...
        # only one set of labels
            ...
            if which_dependent == 'main':
                if add_indeces:
                    i_train = torch.arange(0, self.X_train.shape[0])
                    i_val = torch.arange(0, self.X_val.shape[0])
                    train = TensorDataset(self.X_train, self.y_m_train, i_train)
                    val = TensorDataset(self.X_val, self.y_m_val, i_val)
                else:
                ...
                if add_indeces:
                    i_train = torch.arange(0, self.X_train.shape[0])
                    i_val = torch.arange(0, self.X_val.shape[0])
                    train = TensorDataset(self.X_train, self.y_c_train, i_train)
                    val = TensorDataset(self.X_val, self.y_c_val, i_val)
                else:
...
class multiNLI_Dataset(Dataset_obj):
    ...
    def set_sample_subset(self, n_train, n_val, n_test,  spurious_ratio,p_m,  seed, set_spurious_ratio_same_for_both=True):
        """
        ...
        self.sample_train_ids = self.set_sample_split(self.y_m_train, self.y_c_train, n_train, spurious_ratio, p_m,  seed, set_spurious_ratio_same_for_both=set_spurious_ratio_same_for_both)
        self.sample_val_ids = self.set_sample_split(self.y_m_val, self.y_c_val, n_val, spurious_ratio, p_m,  seed, set_spurious_ratio_same_for_both=set_spurious_ratio_same_for_both)
        self.sample_test_ids = self.set_sample_split(self.y_m_test, self.y_c_test, n_test, 0.5, p_m,  seed, set_spurious_ratio_same_for_both=True)
...
class Waterbird_Dataset(Dataset_obj):
    ...
    def load_dataset(self, spurious_ratio, directory_labels, directory_embeddings, directory_labels_test, directory_embeddings_test, df_meta,df_meta_test, device, model_name='resnet50', image_type_train_val='combined', image_type_test='combined', corr_test='50', adversarial=False, finetuned=False, settings='adversarial', seed=1, balance_main=False):
        """
        ...
        if finetuned:
            train_embedding_name += '_finetuned'
        if finetuned or adversarial:    
            train_embedding_name += '_settings_{}'.format(settings)
        if adversarial:
            train_embedding_name += '_adversarial' 

      
        
        train_embedding_name += '_seed_{}'.format(seed)

        # waterbird_images_combined_test_50_embeddings_resnet50_finetuned_settings_kirichenko_adversarial_settings_kirichenko_seed_1000.pt
        ...
        if finetuned:
            val_embedding_name += '_finetuned'
        if finetuned or adversarial:    
            val_embedding_name += '_settings_{}'.format(settings)
        if adversarial:
            val_embedding_name += '_adversarial' 
        ...
        if finetuned:
            test_embedding_name += '_finetuned'
        if finetuned or adversarial:    
            test_embedding_name += '_settings_{}'.format(settings)
        if adversarial:
            test_embedding_name += '_adversarial' 
       
        
        test_embedding_name += '_seed_{}'.format(seed)

        print('Embeddings (train) loaded from: ', directory_embeddings +'/' + train_embedding_name + '.pt')
        print('Embeddings (test) loaded from: ', directory_embeddings +'/' + test_embedding_name + '.pt')
...
class Toy_Dataset(Dataset_obj):
  ...
  def plot_groups(self,ax_to_add, y_m, y_c, X, 
     ...
     for group in groups:
         ...
         print('X_1_plot shape: ',X_1_plot.shape)
         print('X_2_plot shape',X_2_plot.shape )
         print('group_1 shape ',group_i.shape )
...
class celebA_Dataset(Dataset_obj):
  ...
  def set_dependent_var(self,  main_task_name, concept_name, folder, second_concept_name=None):
    """
    ...
    # get the second concept labels if applicable
    if second_concept_name is not None:
        self.second_concept_name = second_concept_name
        y_c_2 = torch.tensor(df_metadata[second_concept_name].replace({-1:0}).values)
        self.y_c_2 = y_c_2
    ...
    if second_concept_name is not None:
        print('total samples where y_c_2 = 1: ', y_c_2.sum())
        print('total samples where y_m = 1 and y_c_2 = 1: ', (y_m*y_c_2).sum())
        print('total samples where y_m = 1 and y_c_2 = 0: ', (y_m*(1-y_c_2)).sum())
        print('total samples where y_m = 0 and y_c_2 = 1: ', ((1-y_m)*y_c_2).sum())
        print('total samples where y_m = 0 and y_c_2 = 0: ', ((1-y_m)*(1-y_c_2)).sum())
    ...
    if second_concept_name is not None:
        self.y_c_2_train = y_c_2[i_train]
        self.y_c_2_val = y_c_2[i_val]
        self.y_c_2_test = y_c_2[i_test]
  ...
  def create_sample(self, df_metadata, train_size, val_size, test_size, spurious_ratio, seed, p_m=False, set_X=False, combine_sets=True):
        ...
        if combine_sets:
            ...
            if self.second_concept_name is not None:
                self.y_c_2 = torch.cat([self.y_c_2_train, self.y_c_2_val, self.y_c_2_test])
            ...
            if self.second_concept_name is not None:
                y_c_2_after_train_selected = y_c_2[index]
            ...
            if self.second_concept_name is not None:
                y_c_2_after_val_selected = y_c_2_after_train_selected[index]
            ...
            if self.second_concept_name is not None:
                self.y_c_2_train = self.y_c_2[self.sample_train_ids]
                self.y_c_2_val = y_c_2_after_train_selected[self.sample_val_ids]
                self.y_c_2_test = y_c_2_after_val_selected[self.sample_test_ids]
        ...
        # if not combine sets, select from train/val/test separately
            ...
            if self.second_concept_name is not None:
                self.y_c_2_train = self.y_c_2_train[self.sample_train_ids]
                self.y_c_2_val = self.y_c_2_val[self.sample_val_ids]
                self.y_c_2_test = self.y_c_2_test[self.sample_test_ids]
  ...
  def load_embeddings(self, folder_embeddings,  device, file_name= 'celebA_main_task_Blond_Hair_concept_Female_sample_n_images_all.pickle', second_concept=False):
        """
        ...
        if second_concept:
            self.y_c_2_train = data_dict['y_c_2_train'].to(device)
            self.y_c_2_val = data_dict['y_c_2_val'].to(device)
            self.y_c_2_test = data_dict['y_c_2_test'].to(device)
  ...
  def load_dataset(self, transformation_func, main_task_name='Eyeglasses', concept_name='Smiling', sample_n_images=1000, spurious_ratio = 0.5, train_size=800, val_size=100, test_size=100, seed=12345, load_full_images=False, create_embeddings=True, folder='datasets/celebA/raw_input/', device_type='mps', p_m=0.5, finetuned=False, settings=None, adversarial=True, early_stopping=True, balance_main=True, device=None, second_concept_name=None):
    """
    ...
    self.set_dependent_var( main_task_name, concept_name, folder, second_concept_name)
    ...
    if second_concept_name is not None:
        # sense check conditional probabilities
        print('In sample')
        print('Conditional probability of y_m = 1 given y_c_2 = 1 in train: {}'.format(self.y_m_train[self.y_c_2_train==1].float().mean()))
        print('Conditional probability of y_m = 1 given y_c_2 = 0 in train: {}'.format(self.y_m_train[self.y_c_2_train==0].float().mean()))
        print('Conditional probability of y_m = 0 given y_c_2 = 1 in train: {}'.format((1-self.y_m_train)[self.y_c_2_train==1].float().mean()))
        print('Conditional probability of y_m = 0 given y_c_2 = 0 in train: {}'.format((1-self.y_m_train)[self.y_c_2_train==0].float().mean()))
...
def get_dataset_obj(dataset, dataset_settings, spurious_ratio, data_info, seed, device, adversarial=False, use_punctuation_MNLI=False, single_model_for_embeddings=True):
        """
        ...
        seed_model = dataset_settings['finetune_seed']
        if not single_model_for_embeddings:
            seed_model = seed_model + seed
        ...
        if dataset == 'Waterbirds':
            ...
            corr_test = dataset_settings['corr_test']
            ...
            directory_labels_test = dataset_folder + '/' +'Waterbirds/images/waterbirds_{}/tensors'.format(corr_test)
            directory_embeddings_test =dataset_folder + '/' + 'Waterbirds/embeddings/waterbirds_{}_{}'.format(corr_test, dataset_settings['model_name'])
        ...
        # if multiNLI dataset
            ...
            if dataset_settings['spurious_ratio_train'] is not None:
                ...
                filename_data = folder_data + '/'+  'MNLI_data_settings_{}_embedding_type_{}_finetuned_{}_train_size_{}_val_size_{}_spurious_ratio_train_{}_spurious_ratio_sample_{}_binary_{}_dropout_{}_early_stopping_{}_finetune_mode_{}_seed_{}'.format( dataset_settings['finetune_param_type'], dataset_settings['embedding_type'], int(dataset_settings['finetuned_BERT']), dataset_settings['train_size'], dataset_settings['val_size'], int(100*spurious_ratio_train), int(100*spurious_ratio_sample), int(dataset_settings['binary_task']), int(100*dataset_settings['dropout']), int(dataset_settings['early_stopping']),dataset_settings['finetune_mode'], seed_model)
                
            else:
                filename_data = folder_data + '/'+  'MNLI_data_settings_{}_embedding_type_{}_finetuned_{}_train_size_{}_val_size_{}_spurious_ratio_train_{}_spurious_ratio_sample_{}_binary_{}_dropout_{}_early_stopping_{}_finetune_mode_{}_seed_{}'.format( dataset_settings['finetune_param_type'], dataset_settings['embedding_type'], int(dataset_settings['finetuned_BERT']), dataset_settings['train_size'], dataset_settings['val_size'], int(100*spurious_ratio), int(100*spurious_ratio), int(dataset_settings['binary_task']), int(100*dataset_settings['dropout']), int(dataset_settings['early_stopping']),dataset_settings['finetune_mode'], seed_model)

            print('Single model for embeddings: ', single_model_for_embeddings)
            print('Seed model: ', seed_model)
        ...
        # if celebA dataset
            ...
            if dataset_settings['finetuned']:
                

                # Change the filename
                filename_data  = 'celebA_main_task_' + dataset_settings['main_task_name'] + '_concept_' + dataset_settings['concept_name'] + '_sample_n_images_' +str(dataset_settings['train_size']) + '_finetuned_settings_' + dataset_settings['settings'] + '_spurious_ratio_' + str(int(spurious_ratio*100))+ '_early_stopping_' + str(int(dataset_settings['early_stopping']))+  '_adversarial_'  + str(int(adversarial)) + '_seed_' + str(seed_model) 
                print('filename_data: ', filename_data)

                if dataset_settings['second_concept_name'] is not None:
                    filename_data = filename_data + '_second_concept_' + dataset_settings['second_concept_name']
                    data_obj.second_concept_name = dataset_settings['second_concept_name']
                    second_concept = True
                    print('LOADING SECOND CONCEPT')
                else:
                    second_concept = False
                
                filename_data += '.pickle'

                data_obj.load_embeddings( 'datasets/celebA/embeddings/',  device, file_name= filename_data, second_concept=second_concept)

            
            # if black and white original images
            elif dataset_settings['black_and_white_original']:

                 # set the transformation function
                target_resolution = (dataset_settings['image_resolution'], dataset_settings['image_resolution'])
                ...
                sample_n_images = dataset_settings['train_size'] + dataset_settings['val_size'] + dataset_settings['test_size']
                ...
                data_obj.load_dataset(transform_func, main_task_name=main_task_name, concept_name=concept_name, sample_n_images=sample_n_images, train_size=train_size, val_size=val_size, test_size=test_size, spurious_ratio=spurious_ratio, load_full_images=True, create_embeddings=False)
