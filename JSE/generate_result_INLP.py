...
def main(dataset, dataset_setting, spurious_ratio, demean, pca, k_components, alpha,  batch_size, solver, lr,weight_decay,  early_stopping, epochs,balanced_training_concept, balanced_training_main, rafvogel_with_joint, orthogonality_constraint,  per_step, device_type, save_results, seed, folder, baseline_adjust=False, use_standard_ERM_settings_after=True, single_finetuned_model=False):
    ...
    # If rafvogel with joint criteria, get the expected difference and variance of difference
    ...
    V_c_INLP, d_c_INLP = train_INLP(data_obj,
                                                                ...
                                                                epochs=epochs,
                                                                alpha=alpha,
                                                                model_base_name='concept_model_INLP',
                                                                bias=True,
    ...
    data_obj.reset_X(X_train_transformed,X_val_after_proj, batch_size=batch_size)
    ...
    result_per_group_train, _ = get_acc_per_group(main_model(X_train_transformed), y_m_train, y_c_train)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--orthogonality_constraint", type=str, default='False', help = 'run Rafvogel with orthogonality constraint')
    parser.add_argument("--rafvogel_with_joint", default='False', type=str, help='Initialize rafvogel with joint decision rule')
    parser.add_argument("--per_step", type=int, default=1, help="per steps, print the loss")
    parser.add_argument("--device_type", type=str, default="cpu", help="device to use")
    parser.add_argument('--save_results')
    parser.add_argument("--seed", type=int, default=0, help="seed to use")
    parser.add_argument("--folder", type=str, help="which folder")
    parser.add_argument('--single_finetuned_model', type=str, help='whether to use a single finetuned model')
    ...
    main(dataset, dataset_setting, spurious_ratio, demean, pca, k_components, alpha,  batch_size, solver, lr,weight_decay,  early_stopping, epochs,balanced_training_concept, balanced_training_main, rafvogel_with_joint, orthogonality_constraint,  per_step, device_type, save_results, seed, folder, single_finetuned_model=single_finetuned_model)
