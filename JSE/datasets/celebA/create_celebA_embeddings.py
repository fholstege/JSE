...
def main(dataset_setting, sample_n_images, spurious_ratio, seed_list, finetuned=True, settings='adversarial_param', adversarial=True, early_stopping=True, device_type='mps'):
    ...
    for seed in seed_list:
        ...
        if dataset_setting == 'sampled_data_two_concepts':
            second_concept_name = dataset_settings['second_concept_name']
        else:
                                ...
                                folder='raw_input/',
                                  device_type='mps',
                                   finetuned=finetuned,
                                    ...
                                    device=device,
                                    balance_main=False,
                                    second_concept_name=second_concept_name
                                  )
        ...
        if second_concept_name is not None:
            data_dict['y_c_2_train'] = celebA_obj.y_c_2_train
            data_dict['y_c_2_val'] = celebA_obj.y_c_2_val
            data_dict['y_c_2_test'] = celebA_obj.y_c_2_test
        ...
        if finetuned:
            file_name = 'celebA_main_task_' + main_task_name + '_concept_' + concept_name + '_sample_n_images_' +sample_n_images_str + '_finetuned_settings_' + settings + '_spurious_ratio_' + str(int(spurious_ratio*100))+ '_early_stopping_' + str(int(early_stopping))+  '_adversarial_'  + str(int(adversarial)) + '_seed_' + str(seed) 
        else:
            file_name = 'celebA_main_task_' + main_task_name + '_concept_' + concept_name + '_sample_n_images_' +sample_n_images_str 

        if second_concept_name is not None:
            file_name += '_second_concept_' + second_concept_name
        file_name += '.pickle'
