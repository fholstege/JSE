...
def main(settings, finetune_mode, sample_data, train_size, val_size, test_size, spurious_ratio, balance_main, start_seed, sims, device_type='mps', dropout=0.1, adversarial=False, early_stopping=True, use_punctuation=False, binary_task=True):
    ...
    for sim_i in range(sims):
        ...
        if not sample_data:
            ...
            sample = True
            set_spurious_ratio_same_for_both = True
            if spurious_ratio == 0 or spurious_ratio == 1:
                set_spurious_ratio_same_for_both = False
            print('Sample data is: {}'.format(sample))
            print('set spurious ratio same for both is: {}'.format(set_spurious_ratio_same_for_both))
            multiNLI_obj.set_sample_subset(train_size, val_size, test_size, spurious_ratio, 0.5, seed=seed, set_spurious_ratio_same_for_both=set_spurious_ratio_same_for_both)
