...
def get_image_from_vec(embeddings, image_resolution, cmap='gray', img_name='', save=True, show=True,         folder = 'datasets/celebA/illustrated_images/', vmin=None, vmax=None):

        unflatten_func = torch.nn.Unflatten(1, (image_resolution, image_resolution))
        unflattened_image = unflatten_func(embeddings)

        plt.axis('off')
        plt.imshow(unflattened_image.squeeze(0), cmap=cmap, vmin=vmin, vmax=vmax, interpolation='none')
        
        if save:
            plt.savefig(folder +img_name, dpi=300)
...
def return_group_second_concept(y_c, y_m, y_z):
    if y_c == 0 and y_m == 0 and y_z == 0:
        group = 0
    elif y_c == 0 and y_m == 0 and y_z == 1:
        group = 1
    elif y_c == 0 and y_m == 1 and y_z == 0:
        group = 2
    elif y_c == 0 and y_m == 1 and y_z == 1:
        group = 3
    elif y_c == 1 and y_m == 0 and y_z == 0:
        group = 4
    elif y_c == 1 and y_m == 0 and y_z == 1:
        group = 5
    elif y_c == 1 and y_m == 1 and y_z == 0:
        group = 6
    elif y_c == 1 and y_m == 1 and y_z == 1:
        group = 7
    return group
